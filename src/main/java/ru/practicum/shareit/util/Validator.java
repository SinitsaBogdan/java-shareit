...
public class Validator {
    ...
    public static void checkIdInUserRepo(Long id, UserRepository repository) {
        if (id == null) throw new ShareitException(GLOBAL_ERROR__NOT_HEADER_IN_REQUEST);
        if (!repository.checkId(id)) throw new ShareitException(REPOSITORY_ERROR__USER__ID_NOT_IN_REPO__ID);
    }
...
//        if (repository.checkEmailDuplicate(user.getEmail())) throw new ShareitException(USER_ERROR__VALID_DUPLICATE__EMAIL);
    ...
    public static void checkValidItem(@NotNull ItemDto item) {
        if (item.getAvailable() == null) throw new ShareitException(ITEM_ERROR__VALID__AVAILABLE);
        if (item.getName() == null || item.getName().isEmpty()) throw new ShareitException(ITEM_ERROR__VALID__NAME);
        if (item.getDescription() == null || item.getDescription().isEmpty()) throw new ShareitException(ITEM_ERROR__VALID__DESCRIPTION);
    }

    public static void checkValidItemOwner(Long itemId, Long userId, ItemRepository itemRepository) {
        if (!itemRepository.findById(itemId).getOwner().equals(userId)) throw new ShareitException(ITEM_ERROR__VALID__ITEM__USER_NOT_OWNER);
    }
}
